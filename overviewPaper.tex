...
In the previous section we showed that our measurements were collected at the edges and within the region of high column water vapour. 
Still open question: does that mean we successfully sampled the ITCZ, i.e. the transition between dry, subsiding region and moist, ascending region?
To answer this question we will, in the following, show a subset of our measurements in CWV space. 
We choose this representation rather than, e.g. averaging our measurements as a function of latitude, because, as shown by \cite{Mapes2018}, CWV provides a dynamic definition of the moist tropics. 
This means that even when the ITCZ widens or narrows, moves north or south, or breaks up into more complicated structures, we still expect the deep convection to occur within the moistest regions. 

Figure~\ref{fig:itcz_cwv} shows that our measurements successfully sampled the ITCZ. 
In particular, Fig.~\ref{fig:itcz_cwv} (left) shows a simple estimate of cloud occurrence frequency based on the HAMP radar, the profile of relative humidity based on measurements from WALES, together with the upwelling longwave radiative flux measured by BACARDI and the the ice water path (IWP) as calculated from the HAMP radiometers. 
All these are binned and averaged by the CWV measurements from HAMP into \SI{1}{\milli\meter} wide bins, ranging from \SI{44}{\milli\meter} to \SI{61}{\milli\meter}, the 10th and 90th percentile of CWV measurements respectively. 
The very basic estimate of cloud occurrence frequency shown here is calculated as the frequency of the equivalent reflectivity factor being larger than \SI{1e-5}{\milli\meter\tothe{6}\per\meter\cubed}.
As expected for the deep tropics clouds deepen and become more frequent as CWV increases. There is is a transition from a mostly cloud-free free-troposphere (apart from frequent high clouds) to a region of almost 50 $\%$ cloud occurrence frequency throughout the column. 
A small but systematic peak in cloud occurrence frequency 
Note that the small fraction of trade wind clouds for CWV less than \SI{50}{\milli\meter} might be due to the fact that the radar can only detect large enough droplets, that form as clouds deepen, for a more detailed discussion see \cite{konowEURECLtSup2021}. 
% Maybe include some measure from the lidar here.
