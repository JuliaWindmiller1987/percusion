...
shows that as CWV increases, there is is a transition from a mostly cloud-free free-troposphere (apart from frequent high clouds) to a region of almost 50 $\%$ cloud occurrence frequency throughout the column.
Note that the small fraction of trade wind clouds for CWV less than \SI{50}{\milli\meter} might be due to the fact that the radar can only detect large enough droplets, that form as clouds deepen,. 
