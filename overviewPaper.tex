...
\item \item HALO took the most direct route to the EarthCARE ground track after takeoff and remained on it, often for the majority of the flight, always for at least twenty minutes before and after the satellite overpass,
...
We show one day with little deep convection at the edge of the ITCZ and one with frequent deep convection in the centre of the ITCZ. While this might fit well with the mean conception of the ITCZ, we also challenge this view by showing a third day with little deep convection but comparably high CWV in the centre of the ITCZ. This third case highlights that the ITCZ can exhibit markedly different convective states at similar CWV, and thus differ substantially from the mean relationship shown in Fig.~\ref{fig:itcz_cwv}. 
Each case study also shows a snapshot from the limited area model simulations that were run in parallel to the campaign.  
The simulations are described in detail in \cite{fievetTropicalConvectionStormresolving2026} and used the ICON storm-resolving model at 1.25 km resolution. During the campaign, overlapping 48-hour simulations were run, forced by IFS forecasts.
