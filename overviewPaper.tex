...
\begin{itemize}
    ...
    \item Peer-to-Peer Networking: Rather than relying on a single server, every computer in the network can both request and serve data.
\end{itemize}

We started using the peer-to-peer nature for on-site data sharing during the field campaign, and continue using it as it allows to provide data from multiple independent server locations (currently DKRZ, GWDG and other partner labs), without a single point of failure.
...
\begin{figure}
    ...
    \caption{ITCZ in column water vapour space. (left) cloud occurrence frequency from HAMP radar, relative humidity from WALES, CWV from HAMP passiv and longwave radiation from BACARDI, (right) vertical velocity from dropsondes.}
    \label{fig:itcz_cwv}
\end{figure}
