...
\begin{itemize}
    \item Content Addressing: Datasets are identified by a unique, cryptographically secure Content Identifier (CID) derived from the file itself, acting as an immutable fingerprint. Unlike mutable filenames, any change to the data results in a new CID, guaranteeing strict reproducibility for data analysis. The additional option of embedding the CID within citable, permanent DOIs via systems such as DataCite ensures long-term data integrity and scholarly traceability.
    \item Peer-to-Peer Networking: Rather than relying on a single server, every computer in the network can both request and serve data. When a user retrieves or pins a dataset locally, they help host it. Supported by dedicated infrastructure at DKRZ, GWDG, and partner labs, this distributed architecture ensures high redundancy and eliminates single points of failure.
\end{itemize}
...
\begin{itemize}
    \item Figure~\ref{fig:itcz_cwv}: Show that this successfully captures the transition between subsiding / dry / high OLR and ascending / moist / low OLR region (would be nice to include aerosol or humidity plot from WALRES in here!).
\end{itemize}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{figures/itcz_cwv_space.pdf}
    \caption{ITCZ in column water vapour space. (left) cloud occurrence frequency from HAMP and longwave radiation from BACARDI, (right) vertical velocity from dropsondes.}
    \label{fig:itcz_cwv}
\end{figure}
...
Show measurements from HAMP, VELOX, specMACs (see Fig.~\ref{fig:mseRad}).
