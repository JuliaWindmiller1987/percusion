...
Exactly fifty years after GATE, the ORCESTRA (Organized Convection and EarthCARE Studies over the Tropical Atlantic) campaign \citep{stevensORCESTRAOrganizedConvection2026} returned to the tropical Altantic, focusing on mesoscale structures and the processes that govern their formation and evolution.
Taking place from August to September 2024, the campaign combined three research aircraft (\textit{citeBony}), a research vessel (\textit{citeKlocke}) and two observatories, as well as coordinated measurements with the Earth Cloud Aerosol and Radiation Explorer (EarthCARE) satellite \citep{illingworthEarthCARESatelliteNext2015}, launched less than three months before the start of the campaign. 
...
The goal of this section is to provide a brief overview of the data collected during PERCUSION. HALO operated in an updated version of the “flying cloud observatory” configuration originally described by \cite{stevensHighAltitudeLongRangeAircraft2019}. This configuration combines active and passive remote-sensing instruments with in situ measurements from dropsondes, providing a comprehensive view of the atmospheric state, including the thermodynamic and dynamical environment as well as the microphysical properties of clouds and precipitation. In the following, we briefly summarise the measurements described by \cite{stevensHighAltitudeLongRangeAircraft2019} and discuss the modifications and additions implemented for ORCESTRA.

The core remote-sensing configuration aboard HALO consists of a complementary suite of active and passive instruments designed to characterise the thermodynamic and dynamical structure of the tropical atmosphere, as well as its cloud and aerosol composition and radiative properties. A key component is the HALO Microwave Package (HAMP), which combines a suite of passive microwave radiometers with a cloud radar \citep{mechHAMPMicrowavePackage2014, ewaldCalibration35GHz2019}. The passive radiometers consist of three downward-looking modules with 26 frequencies in different bands between 22.24 and 183.31 $\pm$ 12.5 GHz that sample water vapour and oxygen absorption bands, as well as window channels. These measurements provide information on the different condensate phases of water, as well as on their vertically integrated quantities, such as column water vapour, liquid water path and ice water path. The nadir-pointing microwave cloud and precipitation radar operates at 35.5 GHz (Ka band) and is suitable for penetrating columns with heavy precipitation. It measures radar reflectivity, reflectivity-weighted Doppler velocity, and the linear-depolarization ratio to determine target shape.

Additional active remote sensing is provided by WALES (Water Vapour Differential Absorption Lidar), which operates at four wavelengths near 935 nm to deliver high-resolution profiles of the water vapour mixing ratio below the aircraft \citep{wirthAirborneMultiwavelengthWater2009}. Its 532- and 1064-nm aerosol channels, which include depolarisation capabilities, also provide extinction measurements for aerosols and optically thin clouds. 

Radiance and irradiance measurements are obtained using the Spectral Modular Airborne Radiation Measurement System (SMART), which is a suite of passive spectrometers that measure spectral irradiances and radiances in the visible, near-infrared, and shortwave infrared wavelength range (300-2200 nm) \citep{wendischAirborneSpectralAlbedometer2001, wendischACRIDICONCHUVACampaign2016}. These observations are used to infer cloud presence and determine microphysical characteristics, including optical depth and effective droplet radius. 
The spectrometer of the Munich Aerosol Cloud Scanner (specMACS) complements these measurements by providing hyperspectral imaging in the visible/near-infrared (400 to 1000 nm), and shortwave infrared (1000 to 2500 nm) \citep{ewaldDesignCharacterizationSpecMACS2016} to characterize clouds, droplets, and optical thickness. 
This imaging capability is extended into the thermal infrared by VELOX (Video airbornE Longwave Observations within siX channels), which captures high-resolution 2D fields of upward terrestrial radiance across six spectral bands between 7.7 and 12.5 $\mu$m, providing information on cloud top brightness temperature, cloud mask/fraction, and cloud top altitude \citep{schaferVELOXNewThermal2022}.
To quantify the atmospheric energy budget, HALO utilises BACARDI (Broadband AirCrAft RaDiometer Instrumentation), two sets of pyranometers and pyrgeometers measuring upward and downward solar (0.3 to 3 $\mu$m) and thermal-infrared (3 to 100 $\mu$m) irradiances \citep{ehrlichNewAirborneBroadband2023}.

In-situ observations are primarily obtained using dropsondes deployed with the Airborne Vertical Atmospheric Profiling System (AVAPS). Vaisala RD-94 dropsondes provide measurements of temperature, humidity and pressure throughout the atmospheric column. Additionally, for each circle, GPS-derived wind measurements enable horizontal wind profiles to be retrieved, providing area-averaged estimates of large-scale divergence, vorticity and vertical velocity within the sampled air masses \citep{bonyMeasuringAreaAveragedVertical2019}. 
...
\textbf{Missing:}
Updates on VELOX, SpecMACS, dropsondes, new retrievals?
Data handling: IPFS
...
By a generic flight strategy, we mean that flight planning followed a set of objective rules that were applied consistently throughout the campaign. A summary of these rules, together with an example flight, is shown in Fig.~\ref{fig:generic_flight_east}. In particular:
\begin{itemize}
\item flights were scheduled on days when an EarthCARE overpass was within reach,
\item HALO flew back and forth along the EarthCARE ground track, spending at least twenty minutes on the track before and after the satellite overpass,
\item dropsonde circles were placed at the edges of the ITCZ, identified either by the 48 mm contour \citep{Mapes2018} or by the strongest gradient in column water vapour (CWV),
\item a third dropsonde circle was placed in the centre of the ITCZ, defined as the midpoint between the two edge circles,
\item whenever possible, HALO flew at the highest feasible altitude, ideally above the tropical high cloud layer,
\item coordinated measurements with the other observing platforms were carried out whenever possible, including, but not limited to, overflights of and circles around RV METEOR and the SAFIRE ATR-42.
\end{itemize}

\begin{figure}
\centering
\includegraphics[width=1\linewidth]{figures/HALO-20240903a_flight_track.pdf}
\caption{Example flight (flight ID HALO-20240903a) illustrating the generic flight strategy. The flight track is shown in orange, together with a GOES satellite image in the background and column water vapour measurements from the Advanced Microwave Scanning Radiometer (AMSR).}
\label{fig:generic_flight_east}
\end{figure}

The motivation for adopting a generic flight strategy was to obtain a statistically representative sampling of the mesoscale structure of the ITCZ during the campaign period. Flying along the EarthCARE ground track served several purposes. First, as discussed in the introduction, it contributed to the calibration and validation of EarthCARE observations and facilitated the extrapolation of our results to other regions and periods. Second, the repeated back-and-forth pattern along the same track effectively produced three cross-sections of the ITCZ at different times of day: one from EarthCARE and two from HALO. This configuration provides a means of assessing how strongly the mesoscale structure evolves on timescales of a few hours.

The placement of dropsonde circles at the edges of the ITCZ, as defined by column water vapour, was motivated by the desire to connect our observations as directly as possible to previous studies. In particular, the 48 mm CWV contour has been identified as a transition between the deep tropics and the surrounding subsiding regions, while edge intensification refers to precipitation maxima occurring preferentially near this boundary. Vertical velocity measurements in these regions therefore allow for a direct comparison with the mechanisms discussed by \cite{Mapes2018}, \cite{masunagaMechanismMaintenanceSharp2020}, and \cite{masunagaEdgeIntensificationEastern2023}. In addition, the moist margin marks the location where, if present, horizontal upgradient moisture transport by a shallow circulation \citep{Muller2012} is expected to be most efficient.

%In the western basin, the strongest gradients in column water vapour were sometimes oriented predominantly in the east--west direction. At the same time, airspace restrictions made east--west-oriented transects easier to execute. In these cases, the dropsonde circles were therefore aligned in the east--west direction.
...
In total, 23 research flights focused on the Atlantic deep tropics were conducted during ORCESTRA and are shown in Fig.~\ref{fig:flight_overview}. Of these, 11 flights were conducted in the eastern basin near Sal and 11 in the western basin near Barbados, with one transfer flight connecting Sal and Barbados while carrying out targeted ITCZ measurements. Across all flights, 93 circles were flown, 87 of which contained sufficient dropsonde measurements to infer vertical velocity profiles \citep{gloecknerBEACHBarbadosEastern2025}. As described above, each vertical velocity profile represents the area-averaged vertical motion within an individual circle, indicated by the shading in Fig.~\ref{fig:flight_overview}.

Of the 23 research flights, 21 included an EarthCARE underpass. In addition to coordination with EarthCARE, a defining feature of the HALO operations was the extensive coordination with other platforms. These included 16 overpasses of RV METEOR, 14 coordinated measurement events with the SAFIRE ATR-42, four overpasses of the Cape Verde Atmospheric Observatory (CVAO), two overpasses of the Barbados Cloud Observatory (BCO), and two underpasses of the PACE satellite.
...
To illustrate the data collected during the campaign, 
we first show that we systematically sampled the structure of the ITCZ, including vertical profiles of vertical velocity, humidity and aerosols and its edges as defined by CWV. 


We want to give a first overview of the data and show that our measurement strategy worked. 
Overarching goal: we examine how mesoscale structures of clouds, precipitation, surface winds, humidity, and aerosols impact the energy ﬂows, both radiative and through the atmospheric circulation, in the deep tropics. 

First, show that we systematically sampled the ITCZ and its edges, 

\textbf{mesoscale structures of clouds, precipitation, surface winds, humidity, and aerosols}
Compare the edges with the center; measured different mesoscale structures (e.g. different types of cloud systems); different surface wind and aerosols conditions. (MCS, edge-intesification/CWV gradients, doldrums, Saharan Air Layer).
Focus on measurements suitable to study the energy flows: 

\textbf{impact the energy ﬂows, both radiative and through the atmospheric circulation}
Radiation: show that we have the relevant radiation measurements. Show, in an example, how the observed clouds and humidity profiles relate to the observed radiation.
Circulation: show that we have the relevant vertical motion and MSE profiles to study the circulation.
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{figures/mse_radiation.pdf}
\label{fig:mseRad}
...
\centering
\includegraphics[width=
\linewidth]{figures/doldrums.png}
\label{fig:doldrums_draft}
\caption{HALO flight track (black line) and dropsonde locations (black circles) for the flight on the 7th of September 2024. The background shows the horizontal surface wind speed from the limited area model simulations at 16:00 UTC on the same day. Dropsondes with near surface wind speeds less than 3 m/s are filled in white. \textbf{TODO: Would be nice to show some specMACS observations here: can we mark the part of the path with low wind speeds and / or show two images that highlight changes of the specular reflection inside and outside of the doldrums?}}
\end{figure}
